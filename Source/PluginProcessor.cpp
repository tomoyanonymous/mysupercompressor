...
void CompressorAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    mydelaycompdsp.init((int)sampleRate);
}
...
void CompressorAudioProcessor::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
    ...
    const float** inputs = (const float**)alloca(mydelaycompdsp.getNumInputs() * sizeof(float));
    for (int i = 0; i < mydelaycompdsp.getNumInputs(); i++) {
        inputs[i] = buffer.getReadPointer(i);
    }
    
    float** outputs = (float**)alloca(mydelaycompdsp.getNumOutputs() * sizeof(float));
    for (int i = 0; i < mydelaycompdsp.getNumOutputs(); i++) {
        outputs[i] = buffer.getWritePointer(i);
    }
    
    // MIDI timestamp is expressed in frames
    mydelaycompdsp.compute(buffer.getNumSamples(), (float**)inputs, outputs);
}
...
delaycompdsp* CompressorAudioProcessor::getDspInstance(){
    return  &mydelaycompdsp;
}
